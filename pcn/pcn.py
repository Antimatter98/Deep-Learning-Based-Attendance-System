...
def pad_img(img:np.array):
    ...
    ret = cv2.copyMakeBorder(img.numpy(), row, row, col, col, cv2.BORDER_CONSTANT)
    return ret
...
def NMS(winlist, local:bool, threshold:float):
    #print(winlist)
    #winlist = torch.from_numpy(winlist)
    #winlist.cuda()
    #winlist = torch.Tensor(winlist)
    length = len(winlist)
    if length == 0:
        return winlist
...
def stage1(img, imgPad, net, thres):
    ...
    net = net.cuda()
    img = torch.from_numpy(img)
    imgPad = torch.from_numpy(imgPad)
    row = (imgPad.shape[0] - img.shape[0]) // 2
    col = (imgPad.shape[1] - img.shape[1]) // 2
    winlist = []
    netSize = 24
    curScale = minFace_ / netSize
    img_resized = resize_img(img.numpy(), curScale)
        ...
        for i in range(cls_prob.shape[2]): # cls_prob[2]->height
            for j in range(cls_prob.shape[3]): # cls_prob[3]->width
                if cls_prob[0, 1, i, j].item() > thres:
                    ...
                    rw = int(w * sn)
                    if legal(rx, ry, imgPad.numpy()) and legal(rx + rw - 1, ry + rw -1, imgPad.numpy()):
...
def detect(img, imgPad, nets):
    ...
    #print(type(img))
    winlist = stage1(img, imgPad, nets[0].cuda(), classThreshold_[0])
    winlist = NMS(winlist, True, nmsThreshold_[0])
    winlist = stage2(imgPad, img180, nets[1].cuda(), classThreshold_[1], 24, winlist)
    winlist = NMS(winlist, True, nmsThreshold_[1])
    winlist = stage3(imgPad, img180, img90, imgNeg90, nets[2].cuda(), classThreshold_[2], 48, winlist)
